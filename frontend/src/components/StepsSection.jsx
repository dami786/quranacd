      ...
      <div className="max-w-container mx-auto px-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {steps.map(({ step, title, description, cta, Icon }, idx) => (
            <div
              key={step}
              className="bg-[#F4F8F7] rounded-xl p-6 md:p-8 flex flex-col items-center text-center shadow-soft border border-gray-100 hover:shadow-card-hover hover:-translate-y-1 transition-all duration-300 opacity-0 animate-fade-in-up"
              style={{ animationDelay: `${100 + idx * 120}ms`, animationFillMode: 'forwards' }}
            >
              <div className="w-14 h-14 rounded-full flex items-center justify-center mb-4 text-primary bg-primary/15">
